...
#            print(self.getPositionDistribution(self.setGhostPosition(gameState, pos)))
            for newPos, newProb in self.getPositionDistribution(self.setGhostPosition(gameState, pos)).items():
                allPossible[newPos]  += self.beliefs[pos] * newProb
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance)   #weights
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # print("noisyDistance: ", noisyDistance)
        # print("emissionModel: ", emissionModel)
        ...
        #if all particles recieve 0, call initializeUniformly
    ...
    def elapseTime(self, gameState):
        ...
        temp = []
        positionDistribution = util.Counter();
        for pos in self.particles:
            positionDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            temp.append(util.sample(positionDistribution))
    ...
    def getBeliefDistribution(self):
        ...
        beliefs.divideAll(beliefs.totalCount())
        return beliefs
