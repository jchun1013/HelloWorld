...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefs in livingGhostPositionDistributions:
            maxKey = beliefs.argMax()
            maxCounter[maxKey] = beliefs[maxKey]

        distCounter = util.Counter()
        for pos, prob in maxCounter.items():
            distCounter[pos] = self.distancer.getDistance(pacmanPosition, pos)
        keyList = distCounter.sortedKeys()

        closestGhost = keyList[-1]

        actionCounter = util.Counter()
        for action in legal:
            position = Actions.getSuccessor(pacmanPosition, action)
            actionCounter[action] = self.distancer.getDistance(position,closestGhost)

        actions = actionCounter.sortedKeys()
        return actions[-1]
