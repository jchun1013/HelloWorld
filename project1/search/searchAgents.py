    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                ...
                if (state in self.corners) and not (state in currentCorners):
                    visitedCorners = currentCorners + [state]
                    successors.append(((state, visitedCorners), action, cost))
                else:
                    successors.append(((state, currentCorners), action , cost))
...
def cornersHeuristic(state, problem):
    ...
    while cornerLeft:
        ...
        for corner in cornerLeft:
            ...
            c = corner
            cornerList.append((distance, corner))

        distance, corner = min(cornerList)

        heuristic += distance
        currentPosition = corner
        cornerLeft.remove(corner)
...
def foodHeuristic(state, problem):
    ...
    foods = []
    for f in foodList:
        distance = util.manhattanDistance(currentPosition, f)
        foods.append(distance)

    minDistance = min(foods)
    maxDistance = max(foods)
    # difference = maxDistance - minDistance
    diff = util.manhattanDistance(foodList[foods.index(minDistance)], foodList[foods.index(maxDistance)])

    heuristic = minDistance + diff
